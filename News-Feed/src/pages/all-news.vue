...
<div>
 ...
 <div id="loading-more" v-if="showAll && start > 0">
   <button id="forward" @click="goForward" v-if="start < sources.length - 4">LOAD MORE</button>
 </div>

<div class="container" v-if="showOne">
   ...
   <div class="col-md-6 one-fig">
     ...
     <a id="view-website" @click="vienOnWebsite" :href="chosenArticle.url" target="_blank">View On Website</a>
     <blockquote>
     <h1>{{ chosenArticle.title }}</h1><br>
     {{ chosenArticle.description }}
     <br><br><h4 v-if="chosenArticle.author">BY {{ chosenArticle.author }}</h4>
     </blockquote>
   </div>
</div>

<h1 style="color:white;">ALL NEWS</h1>
  ...
  export default {
    ...
    methods: {
      ...
      fetchAllNews() {
        ...
        const obj = this.sources.slice(this.start, this.start + 5);

        for ( const key in obj ) {
          this.axios.get("https://newsapi.org/v1/articles?source="+ obj[key].id +"&sortBy=top&apiKey=3dff5d7c24ad4059bb4c97036fbb30a9")
            .then(({data}) => {

              const obj2 = data.articles;
              ...
              for ( const key in obj2 ) {
                ...
                this.articles.push(obj2[key]);
              }

              this.allNews.push(this.articles);

            })
            .catch(function(e) {
              console.log("There was problem"); // "oh, no!"
            });
        }
